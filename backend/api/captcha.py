...
# macOS Font Paths (Updated to prevent crashes)
FONT_PATH = "/Library/Fonts/Arial.ttf"
FONT_PATH_FALLBACK = "/System/Library/Fonts/Supplemental/Courier New.ttf"
...
def _render_rough_char(char: str, font_size: int, text_color: tuple) -> Image.Image:
    ...
    # Try Arial first, fall back to Courier New
    try:
    ...
    # Draw character
    draw.text((x, y), char, font=font, fill=(*text_color, 255))
    ...
    alpha2 = np.where(
        alpha2 > 80,
        np.minimum(255, alpha2 * 1.4),
        0
    )

    arr2[:, :, 3] = alpha2.clip(0, 255).astype(np.uint8)
...
def create_captcha_image(captcha_text: str) -> Image.Image:
    ...
    # Character positions
    x_positions = [int(width * position) for position in [0.09, 0.25, 0.42, 0.58, 0.75, 0.92]]
    ...
    for i, char in enumerate(captcha_text):
        ...
        char_image = _render_rough_char(char, font_sizes[i], text_color)

        # Horizontal squeeze/stretch
        scale_x = random.uniform(0.90, 1.10)
        char_image = char_image.resize(
            (int(char_image.width * scale_x), char_image.height),
            Image.Resampling.LANCZOS
        )

        # Rotate character
        angle = random.randint(-28, 28)
        rotated = char_image.rotate(angle, expand=True, resample=Image.Resampling.BICUBIC)
        ...
        y = (height // 2 - int(arc_lift) - rotated.height // 2 + random.randint(-4, 4))
    ...
    for _ in range(12):
        x = random.randint(0, width)
        y = random.randint(0, height)
        draw.ellipse((x - 1, y - 1, x + 1, y + 1), fill=(168, 205, 205))
...
def generate_captcha():
    ...
    captcha_store[captcha_id] = {
        "text": captcha_text,
        "expires_at": (datetime.now() + timedelta(seconds=CAPTCHA_EXPIRY)),
        "attempts": 0
    }
    return captcha_id
...
def get_captcha_image(captcha_id: str):
    ...
    return create_captcha_image(data["text"])

# ============================================================
# VERIFY CAPTCHA
# ============================================================

def verify_captcha(captcha_id: str, user_input: str):
    if captcha_id not in captcha_store:
    ...
    if data["attempts"] >= MAX_ATTEMPTS:
        del captcha_store[captcha_id]
        return (False, "Too many attempts. Generate a new CAPTCHA.")
